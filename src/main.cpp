...
std::ifstream in("./bulk_origin.txt");
    ...
    if (in.is_open()) {
        ...
        std::shared_ptr<FileWriter> fileWriter1 = std::make_shared<FileWriter>(FileWriter(of1));
        std::shared_ptr<FileWriter> fileWriter2 = std::make_shared<FileWriter>(FileWriter(of2));
